...
classic_decomp <- wholesale %>% 
  ...
  mutate(`12-MA` = slider::slide_dbl(Index, mean, .before = 5, .after = 6, .complete = TRUE),
         trend = slider::slide_dbl(`12-MA`, mean, .before = 1, .after = 0, .complete = TRUE), #`2x12-MA`
         detrend = Index - trend,
         MONTH = month(yearmonth)
         )  %>% 
  group_by(MONTH) %>%
  mutate(season = mean(detrend, na.rm=T)) %>%
  ungroup()
```
...
# Calculate remainder R = Y - T - S
  mutate(random = Index - trend - seasonal) %>%
  ungroup() %>%
...
classic_decomp %>% 
  ...
  mutate(name = factor(name, levels = c("Index", "trend", "seasonal", "random"))) %>%
