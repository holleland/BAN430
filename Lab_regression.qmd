...
📄 [Print-friendly PDF version](Lab_regression.pdf)
...
leaderboard_score <- function(fc, team, type = c("ex-ante", "ex-post")) {
  type <- match.arg(type)
  acc <- fc %>% accuracy(traffic_ts) %>% slice(1)
  cat("================================\n")
...
Run this once at the top of your session, then call `leaderboard_score(fc_final, "Your team name", type = "ex-ante")` (or `"ex-post"`) on any forecast object — Track A–D or your combined one. Report the printed RMSE, MASE and type in the form below. Resubmitting the same team name **and** type replaces that entry, so submit early and keep improving — a team can hold one ex-ante *and* one ex-post entry at the same time.

</details>

Nobody's checking that your combined model is *novel* — reusing one track's model as-is is a perfectly fine first submission. The point is having a number on the board to improve on.
...
- Revisit the ex-ante/ex-post question from Track D — `moberg` only helps once you're allowed to see it. What would a genuinely ex-ante use of `moberg` look like (a lag? forecasting `moberg` itself first?), and would it actually help?
